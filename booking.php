...
if (!isset($_GET['id'])){
	header('Location:'.$_SERVER["REQUEST_URI"]."?id=6");
}
$event_id = $_GET['id'];
...
$(document).ready(function(){
  ...
  var today = $('#appoint_date').val();
    $.get("get_timeslot.php",{eventid:event_id,date:old_date,time:old_time,today:today,random:Math.random()},function(data){
